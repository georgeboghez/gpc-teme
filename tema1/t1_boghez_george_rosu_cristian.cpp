...
float get_y_coord(float x1,float x2) {
	return sqrt(x1*x1 - (x1 - x2)*(x1 - x2));
}


void Display8() {
	glColor3f(0.2, 0.15, 0.88); // 
	float y_coord = get_y_coord(0.7, 0.35);
   // trasare poligon convex GL_QUADS : (v0,v1,v2, ..., v_{n-1})
	glBegin(GL_POLYGON);
	   glVertex2f(0.7, 0.0);
	   glVertex2f(0.35, y_coord);
	   glVertex2f(-0.35, y_coord);
	   glVertex2f(-0.7, 0.0);
	   glVertex2f(-0.35, -y_coord);
   glVertex2f(0.35, -y_coord);
   ...
   glEnd();
   glColor3f(1, 0.1, 0.1); // rosu
   y_coord = get_y_coord(0.5, 0.25);
   glBegin(GL_POLYGON);
	   glVertex2f(0.5, 0.0);
	   glVertex2f(0.25, y_coord);
	   glVertex2f(-0.25, y_coord);
	   glVertex2f(-0.5, 0.0);
	   glVertex2f(-0.25, -y_coord);
	   glVertex2f(0.25, -y_coord);
   glEnd();
   y_coord = get_y_coord(0.48, 0.24);

   glColor3f(1.0, 1.0, 1.0); // alb
   glBegin(GL_POLYGON);
	   glVertex2f(0.48, 0.0);
	   glVertex2f(0.24, y_coord);
	   glVertex2f(-0.24, y_coord);
	   glVertex2f(-0.48, 0.0);
	   glVertex2f(-0.24, -y_coord);
	   glVertex2f(0.24, -y_coord);
