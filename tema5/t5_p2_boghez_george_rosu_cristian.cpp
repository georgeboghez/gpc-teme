...
public:
	...
	CComplex operator + (CComplex const &obj) {
		CComplex rez(re + obj.re, im + obj.im);
		return rez;
	}
...
std::ostream& operator<<(std::ostream& os, CComplex&obj)
{
	os << obj.getRe() << " + " << obj.getIm() << 'i';
	return os;
}
...
public:
	...
	int isIn(CComplex &x)
	{
		...
		{		
			//z1 = (z0 * z0 + m.c);
			CComplex z1((z0 * z0 + m.c).getRe(), (z0 * z0 + m.c).getIm());
			if (z1 == z0)
			{
...
public:
	...
	int isIn(CComplex &x)
	{
		...
		{
			CComplex z1((z0 * z0 + x).getRe(), (z0 * z0 + x).getIm());
	...
	double normalize_value(double x, double desired_min, double desired_max,  double xmin, double xmax) {
		return (desired_max - desired_min) * (x - xmin) / (xmax - xmin) + desired_min;
	}
	...
	void display(double xmin, double ymin, double xmax, double ymax)
	{
		...
		for (double x = xmin; x <= xmax; x += RX_JF)
			for (double y = ymin; y <= ymax; y += RY_JF)
			{
				...
				{
					//          fprintf(stdout, "   \n");
					double normalized_x = normalize_value(x, -1, 1, xmin, xmax);
					double normalized_y = normalize_value(y, -1, 1, ymin, ymax);
					glVertex3d(normalized_x, normalized_y, 0);
				}
				//else if (r == -1)
				//{
				//	//          fprintf(stdout, "   converge finit\n");
