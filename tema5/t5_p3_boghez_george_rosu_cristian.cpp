...
public:
	...
	CPunct getModifiedPoint(double x_offset, double y_offset) {
		CPunct p;
		p.m.x = m.x + x_offset;
		p.m.y = m.y + y_offset;
		return p;
...
class CSquare {
public:
	void square(double lungime, int nivel, CPunct p, CVector& v) {
		if (nivel == 0) {
			...
			double laturaMica = lungime / 3;
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(2 * laturaMica, 2 * laturaMica), v);
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(-laturaMica, 2 * laturaMica), v);
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(-lungime - laturaMica, 2 * laturaMica), v);
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(-lungime - laturaMica, -laturaMica), v);
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(-lungime - laturaMica, -lungime - laturaMica), v);
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(-laturaMica, -lungime - laturaMica), v);
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(2 * laturaMica, -lungime - laturaMica), v);
			square(lungime / 3, nivel - 1, p_init.getModifiedPoint(2 * laturaMica, -laturaMica), v);
		}
	}
	...
	void afisare(double lungime, int nivel) {
		...
		drawSquare(3 * lungime, p.getModifiedPoint(lungime, lungime), v);

		square(lungime, nivel + 1, p, v);
	}
};
...
class CArboreIntors {
public:
	void arboreIntors(double lungime,
		int nivel,
		double factordiviziune,
		CPunct p,
		CVector v)
	{
		...
		else
		{
			...
			v.deseneaza(p, lungime);
			p1 = v.getDest(p, lungime);
			arboreIntors(lungime * factordiviziune, nivel - 1, factordiviziune, p1, v);
	...
	void afisare(double lungime, int nivel)
	{
		...
		v.deseneaza(p, 0.25);
		p = v.getDest(p, 0.25);
		arboreIntors(lungime, nivel, 0.4, p, v);
	}
};

class CTriHexa {
public:
	void trihexa(double lungime, int nivel, CPunct& p, CVector& v, int d) {
		if (nivel == 0) {

		}
		else
		{
			trihexa(lungime, nivel - 1, p, v, -d);
			v.deseneaza(p, lungime);
			p = v.getDest(p, lungime);
	...
	void afisare(double lungime, int nivel)
	{
		...
		if (nivel % 2 == 0) {
			v1.rotatie(-180);
			trihexa(lungime / (nivel * nivel), nivel, p1, v1, -1);
		}
		else
		{
			v1.rotatie(-120);
